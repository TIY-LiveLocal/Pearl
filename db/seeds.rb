...
File.read(Rails.root.join "db/localbusinesses.txt").each_line do |line|
  ...
  businesses.each do |business|
    ...
    Business.create!(name: business, address: address, city: city, zipcode: zipcode, phone: phone, location: "#{lat},#{long}", image_url: image_url, website_url: url, categories: categories, yelp_id: ) 
    end
  end
